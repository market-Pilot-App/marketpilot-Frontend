...
export default function Boosts() {
  ...
  const saveSettings = async () => {
    setSaving(true);
    try { await api.post("/boost/settings", settings); alert("Settings saved!"); }
    catch { alert("Error saving"); }
    setSaving(false);
  };

  const checkBalance = async () => {
    try { const data = await api.get("/boost/balance"); setBalance(data.balance); }
    catch { alert("Error checking balance"); }
  };

  const runBoosts = async () => {
    try { const result = await api.post("/scheduler/run-boosts"); alert(`Done! Boosted: ${result.boosted} posts`); }
    catch { alert("Error running boosts"); }
  };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
          ...
          <div className="space-y-4">
            <div>
              ...
              <select value={settings.platform} onChange={(e) => setSettings({ ...settings, platform: e.target.value })}
            ...
            <div className="flex items-center gap-3">
              <input type="checkbox" checked={settings.auto_boost_enabled}
                onChange={(e) => setSettings({ ...settings, auto_boost_enabled: e.target.checked })} className="w-4 h-4" />
              <label className="text-sm">Auto-boost enabled</label>
            </div>
            <div>
              <label className="block text-sm text-gray-400 mb-1">Likes per post</label>
              <input type="number" value={settings.likes_per_post}
            ...
            <div>
              ...
              <input type="number" value={settings.views_per_post}
            ...
            <div>
              ...
              <input type="number" value={settings.daily_budget_limit}
        ...
        <div className="space-y-6">
          <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
            ...
            <button onClick={async () => { try { const r = await api.post("/scheduler/boost-tiktok"); alert(`Done! ${r.results?.map((x: any) => x.success ? x.service + ' ✅' : x.service + ' ❌').join(', ')}`); } catch { alert("Error"); } }}
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
            ...
            <button onClick={async () => { try { const r = await api.post("/scheduler/boost-website"); alert(`Done! Orders: ${r.results?.map((x: any) => x.order_id).join(', ')}`); } catch { alert("Error"); } }}
